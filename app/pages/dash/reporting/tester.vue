...
const {
	...
	orderBy: [{ journal: { tranDate: SortOrder.Asc } }, { glAccountNumber: SortOrder.Asc }, { id: SortOrder.Asc }],
	where: { journal: {
		is: {
	...
	glAccount: {
		is: {
			accountType: {
				is: {
					class: { in: [AccountTypeClass.Revenue, AccountTypeClass.Expense] }
				}
			}
		}
	} }
		...
		<UPageGrid class="lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-px">
			<UPageCard class="col-span-3">
				<ClientOnly fallback-tag="span" fallback="Loading...">
					<PivotsLedgerPivot :is-parent-loading="pending" :data="jeData" />
				</ClientOnly>
			</UPageCard>
			<UPageCard title="Info">
				<div>status: {{ String(pending) }}</div>
			</UPageCard>
			<UPageCard title="Raw Data" class="col-span-2">
				<div>
					data:
					<pre>{{ jeData }}</pre>
				</div>
